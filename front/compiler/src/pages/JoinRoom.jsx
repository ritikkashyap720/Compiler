...
function JoinRoom() {
    ...
    useEffect(() => {
        ...
        const init = async () => {
            ...
            // joined users
            socketRef.current.on(Actions.JOINED, ({ clients, userId, name, socketId }) => {
    ...
    return (
      <div className='editorPage dark'>
            <Sidebar users={users}/>
            <Editor onCodeChange={(code) => { codeRef.current = code; }} onInputChange={(input) => { inputRef.current = input; }} onOutputChange={(output)=>{outputRef.current=output}} socketRef={socketRef} roomId={roomId} />
      </div>
